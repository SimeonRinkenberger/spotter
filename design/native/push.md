...
## Findings on the Simulator (iPhone Air, iOS 26.2, Xcode 26.2)

- With the **committed** default entitlements the whole scheme builds, installs and launches:
  `BUILD SUCCEEDED`, no Swift warnings. (Two `appintentsmetadataprocessor` "No
  AppIntents.framework dependency found" warnings are pre-existing on this scheme and come
  from targets this branch does not touch.)
- **`SPOTTER_ENTITLEMENTS = App/Push.entitlements` also builds for the Simulator, on the
  Personal Team.** The simulator signs ad-hoc and never consults a provisioning profile, so
  `aps-environment = development` lands in `App.app.xcent` unchallenged. This matters: the
  owner can exercise the whole enrolment on a simulator **before** paying Apple.
- **A DEVICE build is where the account is actually required**, and the error is exactly the
  one to expect (`-sdk iphoneos`, `generic/platform=iOS`, Personal Team, push entitlement on):
  ```
  error: Provisioning profile "iOS Team Provisioning Profile: com.simeonrinkenberger.spotter.dev"
         doesn't include the Push Notifications capability. (in target 'App')
  error: Provisioning profile "iOS Team Provisioning Profile: com.simeonrinkenberger.spotter.dev"
         doesn't include the aps-environment entitlement. (in target 'App')
  ```
  A control device build with the committed default produced **neither** of those, so they are
  the entitlement's doing and nothing else's.
- **The Simulator does issue an APNs device token.** Measured with a throwaway probe (an
  AppDelegate that took provisional authorization, which grants without a prompt, and then
  called `registerForRemoteNotifications()`; reverted, never committed):
  ```
  Spotter push: PROBE provisional granted=true error=nil
  Spotter push: APNs issued a 160-character device token
  ```
  So `didRegisterForRemoteNotificationsWithDeviceToken` fires here and the plugin's hex
  conversion runs. **160 hex characters is 80 bytes, not the classic 32** — simulator tokens
  are longer than device tokens, which is the argument for `token text` rather than anything
  fixed-width, and for never validating a token's length anywhere.
- `xcrun simctl push` **accepts both committed fixtures** and rejects a payload with no `aps`
  key (`Notification payload is missing the aps key`), so "sent" means Apple's own validator
  read them. The fixtures come out of `npm run push:harness` from the same `apnsPayload()` the
  sender calls, so a simulator cannot show something a phone would not get.
- iOS resolves `spotter://tab/plan` to this app (Settings offers "Open in Spotter?"), so the
  link the payload carries is claimed by the right bundle.
- **Not shown: a banner, and a tap on one.** Provisional authorization delivers quietly to
  Notification Centre by design, and full authorization needs someone to tap Allow on the
  system sheet. This session had no input access to the simulator, so the last two links of
  the chain — the banner, and the tap that routes `userInfo["url"]` into `openDeepLink` — are
  asserted by `tools/ios/push-check.mjs` and `tools/ios/live-state-check.mjs` at each joint
  but were not watched happening.
...
6a. **You can try all of this on a simulator first, today.** Add
   `SPOTTER_ENTITLEMENTS = App/Push.entitlements` to `ios/App/Local.xcconfig`, `npm run
   ios:build`, run it on a simulator: the switch will ask for permission and a sandbox token
   will arrive. Only the send needs the key from step 2, and only a real phone needs step 1.
