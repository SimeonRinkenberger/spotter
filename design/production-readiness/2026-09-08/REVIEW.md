...
- Credential-free release workflow added: isolated PostgreSQL guard tests, real helper-contract regression, transport/admission/merge tests, Deno type check, dependency audit and generated web asset consistency. Official actions pinned by commit. After the final release, main protection was enabled: PRs and the verified GitHub Actions check are required, including for administrators; force pushes and branch deletion are disabled. No second reviewer is mandatory for the sole-maintainer beta.
...
| P1 | Main now requires PRs and passing CI, but staging and rollback verification remain outstanding. | Stage migrations and runtime compatibility tests in a separate project; record known-good commits and a rollback rehearsal. Keep protected AI paused during a failed rollout rather than restoring unguarded spending. Independent review is still needed for security-critical changes. |
...
Keep Luna as the default for text and images. Use Gemini only for the supported raw audio/video path only. Text, coaching and image failures do not trigger Google fallback. Cache trusted complete reads, resume completed slides, avoid paying again for deterministic repairs and keep normal extraction reasoning disabled. Do not switch models based on one failure before distinguishing fetching, provider, parsing and merge failures.
