...
(function () {
  ...
  // Three routes where asking the same question twice at the same moment is not
  // two questions. They are read-shaped — the answer depends only on the body, and
  // the server already caches a demo clip globally — and something now does ask
  // twice: the prefetch that starts when a finger lands on the "?" and the sheet
  // that opens ninety milliseconds later. Sharing the promise costs one AI call
  // instead of two. Everything else on this function is a thing that HAPPENS
  // rather than a thing that is asked — an ingest, a Pumpy turn, a checkout — and
  // two of those are genuinely two.
  ...
  // Two doors lead to boot: onAuthStateChange's first session, and the
  // getSession at the foot of this file for the session that was restored from
  // storage before the listener existed. Both used to open. Measured on the live
  // page: profiles, workouts, collections and collection_items each fetched
  // twice, eight round trips where four would do, and the grid built twice from
  // the same rows. Whoever arrives second now gets the first one's promise.
  var booting = null;

  function boot() {
    if (booting) return booting;
    // Before the network is asked anything. The library the user is looking at is
    // almost always the library they left, so it goes up now and the read that
    // follows corrects it.
    paintCache();
    loadProfile();
    maybeInstallHint();
  ...
  // ---------- cache ----------
  //
  // Measured on the live page with twenty cards: the document is interactive at
  // 77ms and the first card lands at 358ms. Every one of those 280 milliseconds
  // is one PostgREST round trip, not work — the grid itself builds in a
  // millisecond. So the last answer is kept on the phone and painted while the
  // network is still being asked, which is what Instagram and Hevy do and why
  // both feel like they were already open. The read that follows is still the
  // truth; this only decides what is on screen until it arrives.
  //
  // Everything here is wrapped, because localStorage throws rather than returns
  // in a Safari private window and on a phone whose storage is full, and a
  // library that will not load because a cache would not write is a much worse
  // bug than a library that takes 358ms.
  var CACHE_KEY = "spotter-lib-v1";
  // A quota is 5MB on most browsers and the whole point is to be fast; a cache
  // this side of 1.5MB writes in a millisecond or two, and one that has grown
  // past it is not worth the main thread it would cost.
  var CACHE_MAX = 1500000;

  // caption is 62% of an average row and nothing on the library screen reads it
  // — not the card, not the chips, not the search, which looks in titles,
  // exercise names, muscles, equipment and tags. It is dropped, and load()
  // brings the real row back a quarter of a second later.
  function thinRow(w) {
    var out = {}, k;
  ...
  function readCache() {
    try {
      ...
      // Keyed by user, and checked rather than trusted: a phone two people share
      // must never show one of them the other's workouts for a third of a second.
  ...
  function writeCache() {
    ...
    try {
      ...
      // Out of quota, or a browser that refuses to store anything at all. Clear
      // what may be a half-written entry and carry on without a cache.
  ...
  function paintCache() {
    ...
    // Every card in this paint is a card the user has already seen, so not one of
    // them should fly in — and when load() lands a moment later they must not fly
    // in then either. seenCards is exactly the ledger that stops a filter tap
    // re-animating the grid; this is the same idea one launch earlier.
    for (var i = 0; i < state.workouts.length; i++) seenCards[state.workouts[i].id] = 1;
    render();
  }

  // The ninety milliseconds between a finger landing and the tap that follows are
  // already spent — instant.page measures 90ms on touch and 80ms on a mouse — and
  // they are the cheapest ninety milliseconds in the app. The explain sheet's two
  // questions are both fully determined by the exercise under the finger, so both
  // can be asked while the press is still happening; the answers land in the same
  // two caches the sheet reads, so it finds them there and renders nothing twice.
  //
  // The tap that follows asks the same two questions, and api() shares an in-flight
  // answer for exactly these routes, so the pair costs one call rather than two.
  // The only waste left is a press that becomes a scroll instead of a tap, and even
  // that is recovered the moment the user presses the "?" for real. Nothing is
  // prefetched for the swap sheet: a swap needs a reason the user has not chosen
  // yet, and guessing it would spend a credit on the wrong question.
  ...
  function load(retry) {
    return Promise.all([
      ...
      today.at = 0;
      render();
      // A card opened straight off the cache is a card without its caption. The
      // real row is in hand now, so the open one is put right the same way a
      // realtime update puts it right.
      ...
      watchPending();
      // In idle time, because this is for the next launch and the thumbnails of
      // this one are still arriving.
      idle(writeCache, 0);
    });
  }
  ...
  function openDetail(w, keepHistory) {
    ...
    (w.blocks || []).forEach(function (b, bi) {
      ...
      (b.exercises || []).forEach(function (ex, ei) {
        ...
        // Added, not woven in: the press starts the two questions the tap is about
        // to ask, and the tap finds them already in flight.
        help.addEventListener("pointerdown", function () { prefetchExplain(ex, w); });
    ...
    sel.onchange = function () {
      // Optimistic, and now honest about it: patchWorkout's own failure line says
      // "your copy is unchanged", which was not true — the pill stayed switched
      // and no realtime event was coming to correct it, because no row changed.
      var was = w.category;
    ...
    ta.oninput = function () {
      ...
      noteTimer = setTimeout(function () {
        // Nothing is put back in the box on a failure — the user is very likely
        // still typing in it — but the row keeps the last value the server agreed
        // to, so a later render does not show a note that was never saved.
        var was = w.notes;
        var sent = ta.value;
        w.notes = sent;
  ...
  // used to be followed by a card that sat there for a round trip. It closes now.
  // Deleting a collection and deleting a session both already worked this way; the
  // workout itself was the odd one out. There is no undo offered here on purpose —
  // the arming is the confirmation — but a delete the server refuses puts the card
  // back rather than leaving the library quietly wrong.
  ...
  function saveRename() {
    ...
    // A rename is a thing the user typed, so the app already knows the answer and
    // does not need the server's permission to show it. The sheet closes on the
    // tap and the new name is everywhere at once; if the write comes back unhappy
    // the old name goes back and says why — including the duplicate-name answer,
    // which is the one thing here only the server knows.
    if (ctx.kind === "workout") {
  ...
  // The tick, the count and the aria-pressed state all used to wait on a round
  // trip — a fifth of a second in which the row a person had just tapped looked
  // dead, and the honest reading of which is that the tap did not land. Filing a
  // workout under a collection is a thing the phone already knows the answer to,
  // so it happens now and the write catches up. If the write fails the row goes
  // back where it was and says so, which is the only case in which anything moves
  // twice.
    ...
    for (var i = 0; i < 7; i++) {
      (function (i) {
        ...
        rows.forEach(function (p) {
          ...
          x.onclick = function () {
            // A row the server has not confirmed yet has no id to delete by. It
            // goes on its own the moment loadPlan lands.
            if (String(p.id).indexOf("tmp-") === 0) return;
            // Off the day now, then two round trips' worth of confirmation. A row
            // that sits there after being dismissed reads as a tap that missed —
            // and this was also the one write in the file that never looked at
            // r.error, so a failed delete quietly redrew the row still present.
            var kept = state.plan;
  ...
  function openPicker(day, label) {
    ...
    state.workouts.forEach(function (w) {
      ...
      row.onclick = function () {
        ...
        // anything on screen moved. The row wears a temporary id until the real
        // one arrives, and the planx handler knows not to try to delete by it.
