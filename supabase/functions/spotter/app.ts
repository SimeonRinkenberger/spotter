...
(function () {
  ...
  // The nodes of this sheet, kept. Every tap in here changes only what the
  // buttons say and which of them are lit, and this used to answer that by
  // building all of them again: the destination row under the thumb was thrown
  // away and replaced by the very tap that was landing on it. Harmless while the
  // tap had already been delivered — and not harmless at all when the fetch
  // below lands instead, a third of a second after the sheet opens, which is
  // exactly when the first tap is in the air. That tap then had no live button
  // to arrive at and did nothing, and doing nothing twice is what "it freezes".
  ...
  // A thumb drifts while it presses. Eight pixels of that was enough to take the
  // drag off a button, and the release then swallowed the button's click — one
  // lost tap per drift, which is what "the copy sheet freezes" was. A control
  // asks for more travel than the sheet's own body does before the sheet
  // believes the finger meant to throw it away; 24px is still an easy flick from
  // a row, and no longer something a press can do by accident.
