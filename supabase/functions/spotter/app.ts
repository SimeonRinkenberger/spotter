...
(function () {
  ...
  var state = {
    ...
    sounds: true, haptics: true, live: true
  };
  ...
  function startWorkout(w, resume) {
    ...
    stopRest();
    acquireWake();
    // The tap that opened this overlay is the gesture iOS wants before audio.
    liveStart();
    renderWorkout();
    startClock();
  ...
  function renderWorkout(hush) {
    if (!wo) return;
    // Whatever moved the screen moved the card, which is drawn from the session
    // rather than from the DOM below.
    liveDraw();
  ...
  function tickRest() {
    ...
    drawRest(left);
    // The Lock Screen counts down with the ring; nothing leaves until it moves.
    liveDraw();
  ...
  function unlockAudio() {
    // The loop wants the same gesture and is not a sound, so it starts above the
    // mute switch rather than under it.
    liveStart();
  ...
  // ---------- the workout on the lock screen ----------
  //
  // Fitness, Strava, Hevy and Strong put a Live Activity there. ActivityKit is
  // native-only, so this is the nearest true thing the web has: a Now Playing card,
  // whose price is that something must be playing. Six seconds of silence loop while
  // Workout Mode is open and mediaSession writes the words on it. Free with it: the
  // transport buttons become pause-the-rest and skip, and a phone playing audio does
  ...
  function liveStart() {
    if (!liveCan() || !state.live || !wo || wo.finished) return;
    if (!live) {
      var a = el("audio");
      a.loop = true;
      // Made here, not shipped: 16-bit silence is two zero bytes over and over,
      // whose base64 is four characters over and over. Six seconds and not one
      // because Chrome on Android ignores media shorter than five, loop or no.
      a.src = "data:audio/wav;base64,UklGRih3AQBXQVZFZm10IBAAAAABAAEAQB8AAIA+AAACABAAZGF0YQR3AQAA" +
        new Array(32002).join("AAAA");
      // Attached, and never left paused: a detached element gets no audio session,
      // and an iOS web app cannot restart from the Lock Screen a loop that has been
      // paused half a minute. So an interruption — a call — is answered by playing.
      document.body.appendChild(a);
      live = { el: a, line: "", key: "", art: null };
      a.onpause = function () { if (live && wo && !wo.finished) livePlay(); };
      // The only session type allowed past the lock button and the ringer switch.
      // It is also why the music stops, and Settings is the way out of that.
      try { if (navigator.audioSession) navigator.audioSession.type = "playback"; } catch (e) { /* ignore */ }
      // The Lock Screen may only do what the rest strip already does. A remote that
      // could do more would be a second engine to keep in step with this one.
      "play:toggle pause:toggle nexttrack:next previoustrack:prev".split(" ").forEach(function (m) {
        var p = m.split(":");
        try { navigator.mediaSession.setActionHandler(p[0], function () { liveDo(p[1]); }); }
        catch (e) { /* not every action exists everywhere */ }
      });
      // The far end of the wire the worker posts a tapped shade button down.
      if (!liveWired && navigator.serviceWorker) {
        liveWired = true;
        navigator.serviceWorker.addEventListener("message", function (e) {
          if (e.data && e.data.spotter === "live") liveDo(e.data.action);
        });
      }
    }
    livePlay();
  ...
  // Refused outside a gesture, and a refusal costs the card and nothing else. Free
  // while it plays, so every tap in Workout Mode is another attempt.
  function livePlay() {
    var q = live.el.play();
    if (q && q.catch) q.catch(function () { /* no card, no harm */ });
  }

  function liveStop() {
    liveNote(1);
    if (liveCan()) navigator.mediaSession.metadata = null;
    if (!live) return;
    var a = live.el;
    live = null;
    // Pausing is not enough: iOS keeps the card up for a source it can still see.
    try { a.pause(); a.removeAttribute("src"); a.load(); } catch (e) { /* ignore */ }
    document.body.removeChild(a);
  }

  // What Hevy's Live Activity carries, in the room this one has: clock, set or
  // round, movement. Apple's guidance is glanceable-first, so the countdown leads.
  function liveLine() {
    var s = wo.screens[wo.i], bits = [];
    if (restUntil) {
      var n = Math.max(0, Math.ceil((restHeld || restUntil - Date.now()) / 1000));
      bits.push((restHeld ? "Paused" : woPhase === "ready" ? "Get ready"
        : woPhase === "work" ? "Work" : "Rest") + " " +
        (n >= 60 ? Math.floor(n / 60) + ":" + String(n % 60).padStart(2, "0") : n + "s"));
    }
    if (!s) return bits.concat(["Freestyle"]).join(" · ");
    if (isCircuit(s.block)) bits.push("Round " + roundOf(s.bi) + " of " + roundsOf(s.block));
    else if (!restUntil) bits.push("Set " +
      Math.min(wo.entries[wo.i].sets.length + 1, targetOf(s)) + " of " + targetOf(s));
    bits.push(s.ex.name);
    return bits.join(" · ");
  }

  // The rest ticks five times a second, so this is a string compare until something
  // moved: new metadata makes the OS re-read the artwork and the ring must not
  // stutter. Apple asks the same of a Live Activity.
  function liveDraw(force) {
    if (!live || !wo || wo.finished) return;
    // Strava and Hevy both headline elapsed time, and album is the spare line.
    var mins = Math.floor((Date.now() - new Date(wo.startedAt).getTime()) / 60000);
    var line = liveLine(), key = line + "|" + mins;
    if (!force && key === live.key) return;
    live.key = key;
    live.line = line;
    // 512 is what iOS 18 and Chrome's shade both want, and an empty artwork slot
    // reads as a broken app rather than a plain one.
    if (!live.art) live.art = [wo.workout.thumb_url
      ? { src: wo.workout.thumb_url, sizes: "512x512", type: "image/jpeg" }
      : { src: "icon.png", sizes: "512x512", type: "image/png" }];
    try {
      navigator.mediaSession.metadata = new window.MediaMetadata({
        title: wo.workout.title || "Workout", artist: line,
        album: "Spotter · " + mins + " min", artwork: live.art
      });
      // The button says what pressing it does — the rest timer's state, not the
      // silence underneath, which never stops.
      navigator.mediaSession.playbackState = restHeld ? "paused" : "playing";
    } catch (e) { /* an older WebKit simply shows less */ }
    liveNote();
  }

  function liveDo(act) {
    if (!wo || wo.finished || !act) return;
    if (act === "toggle") { if (restUntil) pauseRest(); liveDraw(1); }
    else if (act === "next") {
      if (restUntil) { var then = restThen; stopRest(); if (then) then(); } else skipMove();
    } else if (act === "prev") woGo(-1);
  }

  // Android's shade wants the same sentence: a media notification carries the
  // transport but not the words. One fixed tag makes an update replace the row
  ...
  function liveNote(close) {
    ...
    navigator.serviceWorker.ready.then(function (reg) {
      ...
      reg.showNotification(wo.workout.title || "Workout", {
        tag: LIVE_TAG, body: live.line, icon: "icon.png", badge: "icon.png",
        // Silent, so a ticking rest never lights the screen. Nothing asks for
        // persistence: Android holds a row until it is dealt with.
        silent: true,
        actions: [{ action: "toggle", title: restHeld ? "Resume" : "Pause" },
          { action: "next", title: "Skip" }]
      }).catch(function () { /* a phone that refuses actions still has the card */ });
    }).catch(function () { /* no worker, no shade */ });
  }

  // Once per device, as the timer sounds do it: something appeared outside the app
  // and the sentence explaining it carries the way out — and on Android it is the
  // reason for the prompt, so the tap comes before the dialog.
  var liveTold = false;
  try { liveTold = localStorage.getItem("spotter_live_told") === "1"; } catch (e) { /* ignore */ }
  ...
  function liveTell() {
    ...
    t.onclick = function () {
      t.onclick = null;
      t.classList.remove("tappable");
      try { Notification.requestPermission().then(function () { liveDraw(1); }); }
      catch (e) { /* a browser that will not be asked is not a failure */ }
    };
  }
  ...
  function openSettings() {
    ...
    $("livetoggle").textContent = state.live ? "On" : "Off";
    $("setlivrow").classList.toggle("hide", !liveCan());
    paintSounds();
  ...
  function saveSettings() {
    var s = { unit: state.unit, sounds: state.sounds, haptics: state.haptics, live: state.live };
  ...
  // Off has to take effect on the workout already running underneath this sheet,
  // or the switch is a promise about next time. On cannot: the card needs audio,
  // and audio needs a gesture inside Workout Mode, so it starts at the next set.
  function toggleLive() {
    state.live = !state.live;
    $("livetoggle").textContent = state.live ? "On" : "Off";
    if (!state.live) liveStop();
    saveSettings();
  }
