...
(function () {
  ...
  function loadToday() {
    ...
    Promise.all([
      ...
      // A day either side: eight this morning in Sydney is yesterday in UTC. The
      // window is deliberately loose and sameDay() decides.
      ...
      today.done = (rs[1].data || []).some(function (l) { return sameDay(l.started_at, key); });
      renderToday();
  ...
  // Did this timestamp happen on this local day. started_at is UTC and a day is
  // not: slicing the first ten characters of the ISO string ticks Thursday for a
  // session logged at 19:36 on Wednesday in Chicago, and misses this morning
  // entirely in Sydney. Compare moments, not strings.
  function sameDay(iso, key) {
    if (!iso) return false;
    var t = new Date(iso);
    return !isNaN(t.getTime()) && ymd(t) === key;
  }
  ...
  function loadPlan(silent) {
    ...
    // A day before Monday, because a Monday-morning session east of UTC is stored
    // with Sunday's date. sameDay() below decides which day each one lands on.
    var fromD = new Date(state.weekStart.getTime() - 86400000);
    return Promise.all([
      sb.from("plan").select("*").gte("day", start).lte("day", ymd(endD)),
      sb.from("workout_logs").select("id,started_at").gte("started_at", ymd(fromD) + "T00:00:00Z")
    ]).then(function (rs) {
  ...
  function renderPlan() {
    ...
    for (var i = 0; i < 7; i++) {
      (function (i) {
        ...
        var didLog = (state.planLogs || []).some(function (l) { return sameDay(l.started_at, key); });
