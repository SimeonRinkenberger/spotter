...
(function () {
  ...
  function clearAccount() {
    ...
    ["grid", "chips", "colbar", "libcount", "empty", "dinner", "pumpylog", "pumpyannounce", "pumpyctx", "pumpythreads", "trainview", "pausedbar", "recapopts"].forEach(function (id) {
      var n = $(id); if (n) n.innerHTML = "";
    });
    $("countlib").textContent = "Reading your workouts";
  ...
  function signedOut() {
    ...
    state.user = null;
    guideUser();
    state.workouts = []; state.logs = null; state.logsLite = false; state.libReady = false;
    state.plan = null; state.awards = null;
  ...
  function boot() {
    ...
    earlyUid = null;
    // Train is the page the app opens on, so its two reads go out now, beside the
    // library's, rather than in idle time after it.
    drawn.train = true;
  ...
  // ---------- Train's cache ----------
  //
  // Train is the first page now, and after an hour away it would otherwise wait
  // on two reads — the plan and the logs — before it could say what today is.
  // So the last answer is kept, as the library's is: the plan rows, and the logs
  // cut down to what the strip, the ring, the statuses and Up next read (which
  // workout, when, how long, how many sets — each set a bare 1). state.logsLite
  // says the logs are that cut; loadLogs treats them as not loaded, and anything
  // that needs a log's real figures (Progress, Records, awards, a corrected best)
  // asks fullLogs(), which answers null until the real ones land.
  var TRAIN_KEY = "spotter-train-v1", trainCacheTimer = 0, trainCacheGoal = null;

  function liteLog(l) {
    var ones = [];
    (l.entries || []).forEach(function (e) { (e.sets || []).forEach(function (x) { if (x) ones.push(1); }); });
    return { id: l.id, workout_id: l.workout_id, workout_title: l.workout_title, started_at: l.started_at,
      completed_at: l.completed_at, duration_seconds: l.duration_seconds, entries: ones.length ? [{ sets: ones }] : [] };
  }

  function fullLogs() { return state.logsLite ? null : state.logs; }

  function writeTrainCache() {
    if (!state.user) return;
    clearTimeout(trainCacheTimer);
    var uid = state.user.id;
    trainCacheTimer = setTimeout(function () {
      if (!state.user || state.user.id !== uid || !state.plan || !state.logs || state.logsLite) return;
      try {
        localStorage.setItem(TRAIN_KEY, JSON.stringify({ v: 1, uid: uid, at: Date.now(),
          goal: goalSetting(), plan: state.plan, logs: state.logs.map(liteLog) }));
      } catch (e) { try { localStorage.removeItem(TRAIN_KEY); } catch (e2) { } }
    }, 1000);
  }

  // Only into an empty slot: a real answer already in hand is never replaced by
  // an older one, and the reads that follow overwrite this one as they land.
  function paintTrainCache(uid) {
    var c = null;
    try { c = JSON.parse(localStorage.getItem(TRAIN_KEY) || "null"); } catch (e) { c = null; }
    if (!c || c.v !== 1 || !uid || c.uid !== uid) return false;
    if (!state.plan && c.plan) state.plan = c.plan;
    if (!state.logs && c.logs) { state.logs = c.logs; state.logsLite = true; }
    trainCacheGoal = c.goal || null;
    return true;
  }
  ...
  function paintRows(c) {
    ...
    render();
    // The first page, drawn from the same kept answer before any read returns.
    if (state.plan && state.logs) renderTrain();
  }
  ...
  function load(retry) {
    ...
    return readOnce("library:" + rev + ":" + !!retry, function () {
      ...
      return Promise.all([rows, collections]).then(function () {
        ...
        toast("Could not load your workouts — pull down to try again.");
      });
    });
  }
  ...
  // Three different things can be happening to a card that has not landed yet, and
  // a card that says "Reading" while Spotter is listening to the audio is telling
  // the user something that is not true. The server sets media_stage on the row as
  ...
  // "not in the caption" is true of every exercise read off a screen and tells the
  // user nothing they can act on, while "read off the screen" tells them exactly
  // what to check and where to look.
  function readFrom(w) {
    var out = { video: false, speech: false };
    (w.blocks || []).forEach(function (b) {
      (b.exercises || []).forEach(function (ex) {
        var src = ex.evidence && ex.evidence.source;
        if (src === "video") out.video = true;
        if (src === "transcript") out.speech = true;
      });
    });
    return out;
  }

  // Whose videos Spotter can actually reach. Measured 2026-09-02: TikTok's watch
  // page still names the media file and hands out the cookies that make it
  // fetchable, and Instagram names no media URL at all — not to a server, not to
  // a phone on its own connection — so offering this on a reel would be offering
  // a button that can only fail.
  function canReadVideo(w) {
  ...
  //
  // Filters narrow and sort re-shapes, as two controls, which is the split Spotify's
  // Your Library and Pinterest's boards keep. Newest leads: it is today's order.
  var SORTS = ["new", "by", "mg"];
  ...
  // Back, Biceps, Calves, which is a filing cabinet rather than a body.
  ...
  // nobody signed it, no heading claimed it — because a section a person can see
  // has to be one they can tap, leftovers included.
  ...
  //
  // Where a heading rests: under the app header and the search bar, the two things
  // this page scrolls beneath. Measured, not written into the stylesheet, because
  // one grows by the status bar and the other by the text size.
  function secTop() {
  ...
  function openSort(chose) {
    ...
    SORTS.forEach(function (k) {
      ...
      // Apple's Menus guidance: a checkmark is what people scan a list of attributes
      // for, so the one in force wears a mark rather than a lit background.
  ...
  function setSort(k) {
    ...
    if (k !== sortMode) {
      ...
      // A narrow taken out of a section means nothing under another sort.
  ...
  function renderGrid() {
    ...
    if (!items.length) {
      ...
      if (!state.workouts.length) {
        ...
        var first = el("button", "btn firstsave", "Add video");
    ...
    // Grouping goes off once the grid is narrowed to one of its own sections: one
    // heading says nothing the line above it just said. A category or a collection
    // still groups — "Push, by creator" is a question people ask.
  ...
  function cardNode(w, i) {
    ...
    if (w.favorite) {
      ...
      fv.setAttribute("aria-label", "Favorite");
      tw.appendChild(fv);
    }
  ...
  function bitBtn(w, ex) {
    ...
    var b = icon(el("button", "chip"), "play", "Watch this part");
  ...
  // One round trip, not two: thread then messages left the column blank for two
  ...
  // How long a cold list may keep the sheet still, and a tapped thread may keep it
  // open. Both inside the second Nielsen calls the limit of held thought.
  var TH_WAIT = 220, OPEN_WAIT = 320;

  // Draw first, move second. The sheet used to start on one line of text and take
  // its rows 180-280ms later: anchored to the bottom edge, its TOP jumped 187.8 to
  // 344.9px with three chats, to 714.6 with fifty, both inside the 380ms spring —
  // it grew out from under the thumb. No long task was recorded either time, so it
  // was never main-thread cost. The list drawn last time is true enough to open at;
  ...
  function openPumpyThreads() {
    ...
    function present(rows, err) {
      ...
      pumpy.opening = false;
      clearTimeout(timer);
      renderThreads(rows, err);
      openSheet("pumpysheet");
      pumpy.sheetAt = now();
    }
    // Behind an opening sheet a matching refresh is two strings in a fixed box and
    // moves nothing; anything else waits for the spring.
    function land(rows, err) {
      if (opened && (err || !patchThreads(rows))) sheetSettled(function () { renderThreads(rows, err); });
      else present(rows, err);
    }
    if (pumpy.threads) present(pumpy.threads);
    else { pumpy.opening = true; timer = setTimeout(function () { present(null); }, TH_WAIT); }
    sb.from("pumpy_threads")
  ...
  function rowBits(t) {
    var bits = [], a = agoText(t.updated_at);
    if (a) bits.push(a);
    if (t.workout_id) bits.push("About " + ((t.workouts && t.workouts.title) || "a workout"));
    if (threadId() === t.id) bits.push("Open");
    return bits.join(" · ");
  }

  function threadRow(t) {
    var row = el("div", "threadrow" + (threadId() === t.id ? " on" : ""));
    row.setAttribute("data-t", t.id);
    row.thread = t;   // patchThreads rewrites this, so the handlers read it
    var main = el("button", "tmain");
    main.appendChild(el("b", null, t.title || "New chat"));
    main.appendChild(el("span", null, rowBits(t)));
    main.onclick = function () { openThread(row.thread, row); };
    ...
    del.onclick = function () {
      armed(del, "Delete?", function () {
        sb.from("pumpy_threads").delete().eq("id", t.id).then(function (r) {
          ...
          // The cache is what the next open draws before it moves, so it forgets
          // the row too, or the chat is back for a frame.
          pumpy.threads = (pumpy.threads || []).filter(function (x) { return x.id !== t.id; });
          if (threadId() === t.id) newPumpyThread();
          if (!$("pumpythreads").querySelector(".threadrow")) renderThreads([]);
        });
      });
    };
    row.appendChild(del);
  ...
  // rows null is "never asked yet": three grey rows at the REAL row height, so even
  // a first open takes a plausible size before it moves. Seen only past TH_WAIT —
  // under a second, no loading state at all is the researched answer.
  function renderThreads(rows, err) {
    var list = $("pumpythreads"), f = document.createDocumentFragment(), i;
    if (!rows) {
      for (i = 0; i < 3; i++) {
        var s = el("div", "threadrow skel"), m = el("div", "tmain");
        m.appendChild(el("b"));
      ...
      if (err) f.appendChild(el("div", "threadnone", "Could not load your chats — try again in a moment."));
      else if (!rows.length) f.appendChild(el("div", "threadnone",
  ...
  // True when the refresh matched the rows on screen one for one, the usual answer.
  function patchThreads(rows) {
    var have = $("pumpythreads").querySelectorAll(".threadrow[data-t]"), i;
    if (have.length !== rows.length) return false;
    for (i = 0; i < rows.length; i++) if (have[i].getAttribute("data-t") !== rows[i].id) return false;
    for (i = 0; i < rows.length; i++) {
      have[i].thread = rows[i];
      have[i].querySelector("b").textContent = rows[i].title || "New chat";
      have[i].querySelector(".tmain span").textContent = rowBits(rows[i]);
      have[i].classList.toggle("on", threadId() === rows[i].id);
    }
    return true;
  }

  // iOS Messages never shows an empty conversation on the way to a full one: UIKit
  // gives it viewIsAppearing(), laid out but not yet seen, expressly so the scroll
  // can be set without a jump. A sheet closing over 260ms under a 55% scrim is the
  ...
  function openThread(t, row) {
    ...
    if (row) row.classList.add("on");   // the tap's own answer, before a byte lands
    // A slow network does not hold the sheet open. Past OPEN_WAIT it goes and the
    // thread you were reading stays, quiet, until its replacement is ready: an old
  ...
  function renderRefList() {
    ...
    if (!rows.length) {
      list.appendChild(el("p", "lede", state.workouts.length ? "Nothing by that name."
        : "Add a workout video first, then you can point Pumpy at it."));
    }
    rows.forEach(function (w) {
  ...
  function paintPlanUse(r) {
    ...
    if (lim && capNum(lim.library) !== null && num(r.library_count) !== null) {
      n.appendChild(useRow("Workouts", r.library_count + " of " + capNum(lim.library), " saved", null,
  ...
  var GUIDE_TIPS = {
    ...
    save: { title: "Save it now. Train it later.", art: "coach",
      text: native ? "Share it from TikTok or Instagram, or paste its link below. You can leave while I read it."
        : "Paste the workout link, then tap Add video. You can leave while I read it." },
    detail: { title: "Make this workout yours", art: "coach",
      text: "Tap an exercise to watch that part, see a demo, edit it or swap it." },
    ...
    train: { title: "Your week, in one place", art: "plan",
      text: "Swipe the days to change week, pull the strip down for the month. Progress and Records live below." },
  ...
  function openWelcome() {
    ...
    $("welcomenext").textContent = "Add a video";
  ...
  // A thumb drifts while it presses, and eight pixels of drift was enough to
  // take the drag off a button and have the release swallow that button's click:
  // one lost tap per drift, the other half of the copy sheet's freeze. So a
  // control asks 24px before the sheet believes the finger meant to throw it
  // away — still an easy flick, no longer something a press does by accident.
  var SH_FLING = 350, SH_PART = 0.35, SH_TAP = 24;
  ...
  // ---------- strava ----------
  //
  // The only thing in Spotter that writes to somebody else's account.
  //
  // A new Strava API app may connect exactly ONE athlete until the owner
  // self-upgrades to ten, and past ten is a review — so the whole group is hidden
  // until the function says configured, and the line under the button says
  // "limited beta" rather than letting somebody find that out at a consent screen.
  // No token comes near this file: the browser asks for a URL, walks to
  // strava.com, comes back to a static page, and holds three booleans after it.

  var STRAVA_ORANGE = "#FC5200";
  var STRAVA_KEY = "spotter_strava_return";
  var STRAVA_WAIT = "spotter_strava_waiting";

  var strava = { asked: false, configured: false, connected: false, athlete: null, busy: false };

  // captureBilling's shape for captureBilling's reason: whatever came back on the
  // address bar must be off it before anything reloads, and there may be no
  // session yet when this runs.
  function captureStrava() {
    ...
    try {
      sessionStorage.setItem(STRAVA_KEY, JSON.stringify({
        outcome: q.get("strava"), why: q.get("why") || null
      }));
    } catch (e) { /* ignore */ }
    try { history.replaceState(null, "", location.pathname + location.hash); } catch (e) { /* ignore */ }
  }

  // One vocabulary for both halves: the codes the callback puts on the address bar
  // and the codes a push answers with overlap, and one wall deserves one sentence.
  function stravaSays(code, message) {
    if (code === "capacity") return "Strava lets this app connect a limited number of athletes right now.";
    if (code === "disconnected") return "Strava disconnected this app — reconnect in Settings.";
    if (code === "not_connected") return "Connect Strava in Settings first.";
    if (code === "declined") return "Strava was not connected. You can try again any time.";
    if (code === "scope") return "Spotter needs permission to add workouts to Strava — try again and leave that box ticked.";
    if (code === "bad_state") return "That took too long. Tap Connect with Strava again.";
    return message || "Strava could not take that just now. Try again in a minute.";
  }

  function stravaWait(on) {
    try {
      if (on) sessionStorage.setItem(STRAVA_WAIT, "1"); else sessionStorage.removeItem(STRAVA_WAIT);
    } catch (e) { /* ignore */ }
    return on;
  }

  function consumeStrava() {
    var raw = null, o = null;
    try {
      raw = sessionStorage.getItem(STRAVA_KEY);
      if (raw) sessionStorage.removeItem(STRAVA_KEY);
    } catch (e) { /* ignore */ }
    try { o = raw ? JSON.parse(raw) : null; } catch (e) { o = null; }
    if (!o) return false;
    stravaWait(false);
    strava.asked = false;
    toast(o.outcome === "connected" ? "Strava connected." : stravaSays(o.why, null), 4200);
    // A return that lands before the session is restored still gets its toast; the
    // status read waits for somebody to ask for it.
    if (state.user) askStrava();
    return true;
  }

  // Three doors onto one question, the three billing already needed: pageshow,
  // visibilitychange, focus. On iOS the consent screen opens in a browser whose
  // storage is not ours and the return address may never reach the app at all, so
  // a connect that never came back is settled by asking the server instead.
  function stravaBack() {
    if (document.visibilityState === "hidden") return;
    if (consumeStrava()) return;
    var waiting = false;
    try { waiting = sessionStorage.getItem(STRAVA_WAIT) === "1"; } catch (e) { /* ignore */ }
    if (!state.user || !waiting) return;
    stravaWait(false);
    strava.asked = false;
    askStrava().then(function () { if (strava.connected) toast("Strava connected."); });
  }

  // Once per page load unless something happened that could change the answer.
  // configured:false is a 200 rather than an error, so a project with no Strava
  // secrets draws no group and shows no failure.
  function askStrava() {
    if (strava.asked) return Promise.resolve(strava);
    strava.asked = true;
    return api("strava/status", { method: "GET" }).then(function (r) {
      if (r && r.status === "ok") {
        strava.configured = !!r.configured;
        strava.connected = !!r.connected;
        strava.athlete = r.athlete_id || null;
      }
      paintStrava();
      return strava;
    }).catch(function () {
      // Ask again next time rather than hide the group for ever over one dropped
      // request.
      strava.asked = false;
      paintStrava();
      return strava;
  ...
  function paintStrava() {
    var box = $("setconn");
    if (!box) return;
    box.classList.toggle("hide", !strava.configured);
    if (!strava.configured) return;
    var on = strava.connected;
    $("setstrava").textContent = on ? "Connected" : "Not connected";
    $("stravaon").classList.toggle("hide", on);
    $("setstravaoffrow").classList.toggle("hide", !on);
    $("setstravanote").textContent = on
      ? "Finished sessions can be sent to Strava as a manual workout: its name, how long it took, and " +
        "your sets in the description." + (strava.athlete ? " Athlete " + strava.athlete + "." : "") +
        " Disconnecting takes Spotter's access away and deletes the tokens."
      : "Limited beta. Strava caps how many people one app may connect, so this may not be open to your " +
        "account yet. Spotter only writes: it adds a finished session and never reads anything back.";
  }

  function connectStrava() {
    var b = $("stravaon");
    b.disabled = true;
    api("strava/connect", { method: "GET" }).then(function (r) {
      if (!r || r.status !== "ok" || !r.url) {
        b.disabled = false;
        toast((r && r.message) || "Could not start the Strava connection.");
        return;
      }
  ...
  // Armed, like every other undoing in the app. Disconnecting is reversible, but
  // it is not a thing anybody means to do with a stray thumb.
  function stravaOff(btn) {
    armed(btn, "Tap again to disconnect", function () {
      btn.disabled = true;
      api("strava/disconnect", { method: "POST", body: "{}" }).then(function (r) {
        btn.disabled = false;
        if (!r || r.status !== "ok") { toast((r && r.message) || "Could not disconnect just now."); return; }
  ...
  // Strava asks for the way back to be findable — bold, underline or their orange.
  // An anchor because it leaves the app, wearing the button's geometry so it sits
  // level with whatever the share row put beside it.
  function stravaLink(id) {
    var a = el("a", "btn ghost", "View on Strava");
    a.href = "https://www.strava.com/activities/" + id;
    a.target = "_blank";
    a.rel = "noopener";
    a.style.cssText = "color:" + STRAVA_ORANGE +
      ";font-weight:700;display:block;box-sizing:border-box;text-align:center;text-decoration:none";
    return a;
  }

  // start_date_local is the session's LOCAL wall clock and only the phone knows
  // it. The trailing Z is part of the format Strava documents, not a claim about
  // UTC — sending a real UTC stamp files a 6pm session at 11pm.
  function stravaLocalISO(iso) {
    var d = iso ? new Date(iso) : new Date();
    if (isNaN(d.getTime())) d = new Date();
    return new Date(d.getTime() - d.getTimezoneOffset() * 60000).toISOString().slice(0, 19) + "Z";
  }

  // The bests read off the sets themselves rather than out of Workout Mode's
  // tally, so a card in History composes the same description as the summary did.
  function stravaPrs(log) {
    var out = [];
    (log.entries || []).forEach(function (e) {
      (e.sets || []).forEach(function (s) {
        if (s && s.pr) out.push({ name: e.name, weight: s.weight, unit: s.unit, reps: s.reps });
      });
    });
    return out.slice(0, 6);
  }

  function pushStrava(log, b) {
    ...
    api("strava/push", {
      method: "POST",
      body: JSON.stringify({
        log_id: log.id, start_local: stravaLocalISO(log.started_at),
        unit: state.unit, prs: stravaPrs(log)
      })
    }).then(function (r) {
      strava.busy = false;
      if (r && r.status === "ok" && r.activity_id) {
        // Remembered on the row as well as on Strava, so leaving History and
        // coming back shows the link rather than offering the send again.
        log.strava_activity_id = r.activity_id;
        if (b.parentNode) b.parentNode.replaceChild(stravaLink(r.activity_id), b);
        haptic("success");
        toast("Sent to Strava.");
        return;
      }
      b.disabled = false;
      ...
      toast(stravaSays(r && r.code, r && r.message), 4200);
    }).catch(function () {
      strava.busy = false;
      b.disabled = false;
      stravaSwap(b, "Send to Strava");
  ...
  // The hook the share row calls, on the summary and on every History card. Null
  // whenever Strava is off or not connected, which keeps a feature nobody can use
  // out of everybody else's session summary.
  function stravaBtn(log) {
    if (!strava.configured || !strava.connected || !log) return null;
    if (log.strava_activity_id) return stravaLink(log.strava_activity_id);

    var b = el("button", "btn ghost", "Send to Strava");
    b.style.color = STRAVA_ORANGE;
    b.style.fontWeight = "700";
    b.onclick = function () { pushStrava(log, b); };

    // The summary draws the instant the workout ends, before the insert has come
    // back with the id a push has to be aimed at. So the button says so, watches
    // the object finishWorkout stashes it onto, and wakes when it lands — for ten
    // seconds, then it stops claiming a save that plainly did not happen.
    if (!log.id) {
      b.disabled = true;
  ...
  $("stravaon").onclick = connectStrava;
  $("stravaoff").onclick = function () { stravaOff(this); };

  // The official image is the required entry point to Strava's consent screen and
  // may not be redrawn, so if the asset has not landed in docs/ yet the same
  // button says the words instead of leaving an invisible control on the page.
  $("stravaonimg").onerror = function () {
    var b = $("stravaon");
    if (this.parentNode === b) b.removeChild(this);
    b.textContent = "Connect with Strava";
    b.style.cssText = "width:100%;border:0;border-radius:14px;padding:14px;font-size:15.5px;" +
      "font-weight:700;background:" + STRAVA_ORANGE + ";color:#FFFFFF";
  };

  window.addEventListener("pageshow", stravaBack);
  window.addEventListener("focus", stravaBack);
  document.addEventListener("visibilitychange", stravaBack);
  ...
  // Train first: the app opens on what to do today, and Workouts is the shelf it
  // is chosen from (owner decision, 25 Sept, Option B "Train is home").
  var VIEWS = ["train", "library", "pumpy"];
  var PAGE_IDS = ["trainview", "libpage", "pumpyview"];
  ...
  function commit(i) {
    ...
    idx = i;
    state.view = VIEWS[i];
    // The header's two per-page parts: Train's streak and ring are tappable only
    // on Train, and the add button wears its word only on Workouts.
    hdrEl.classList.toggle("ontrain", state.view === "train");
    hdrEl.classList.toggle("onlib", state.view === "library");
