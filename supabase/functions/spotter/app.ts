...
(function () {
  ...
  function scDraw(c, d, p, face, marks, t) {
    ...
    a = scSeg(t, 0, 0.18);
    c.globalAlpha = a;
    c.drawImage(sm, 0, (a - 1) * 90, SC_W, SC_H);
    c.globalAlpha = 1;
  ...
  function scAssets(card) {
    ...
    return Promise.all([scFace(),
      ...
      return { p: p, face: r[0], marks: { mark: r[1], coach: r[2] } };
    });
  }
  ...
  function scArm() {
    if (!sc.vbtn) return;
    if (sc.vst === 2) { sc.vbtn.parentNode.removeChild(sc.vbtn); sc.vbtn = null; return; }
  ...
  function scButtons(btns) {
    ...
    if (can === 2) {
      ...
      sc.hint.textContent = "This browser can only share a link. Save the image and post it yourself.";
    } else {
      btns.appendChild(scBtn("Download", "btn", null, scSave));
      if (window.ClipboardItem && navigator.clipboard && navigator.clipboard.write) {
        btns.appendChild(scBtn("Copy image", "btn ghost", null, scCopy));
      }
      sc.hint.textContent = "Download it, or copy it straight into a post.";
    }
    if (sc.vst !== 2) {
      sc.vbtn = scBtn("Making video…", "btn ghost", null, scVideo);
      btns.appendChild(sc.vbtn);
      scArm();
    }
  ...
  function shareRow(payload, logged) {
    ...
    [["dark", "Dark"], ["light", "Light"]].forEach(function (t) {
      ...
      b.onclick = function () {
        ...
        pair.forEach(function (x) { x[0].setAttribute("aria-pressed", x[1] === sc.bg ? "true" : "false"); });
        haptic("tap");
        scPaint();
      };
      pair.push([b, t[0]]);
      b.setAttribute("aria-pressed", sc.bg === t[0] ? "true" : "false");
      chips.appendChild(b);
    });
