...
(function () {
  ...
  // one row where the numbers shout, the size follows the text — measured against
  // where a counting figure lands, so it cannot shrink under itself mid-clip.
  ...
  // The clip's whole motion vocabulary: how far a window between two moments has
  // got, eased out, and a figure counted up to it. "bodyweight" is not a number
  // and stands still.
  ...
  // finished card, which is the PNG and the clip's last two seconds both.
  function scDraw(c, d, p, face, marks, t) {
    var L = SC_PAD, R = SC_W - SC_PAD, W = R - L, i, a;
    // Clear is the overlay asset: shorter, no ground, and dropped on top of the
    // poster's own clip in the Story editor. Photo is their own picture under the
    // usual card — never the creator's clip frame, which is a third party's work.
    ...
    if (photo) {
      // Cover-cropped, because the poster framed it; a wash and a scrim down the
      // lower half so white text survives a bright gym.
    ...
    // Over a photo, and over whatever a Story editor puts behind a transparent
    // card, every run of text has to bring its own ground with it.
    if (clear || photo) { c.shadowColor = "rgba(0,0,0,.55)"; c.shadowBlur = 20; }

    // Each row arrives a beat after the one above it while the clip runs, and in
    // the still they are all simply there. One call fades the next row in and says
    // how far it still has to rise.
    var n = 0;
    function scStep() {
    ...
    // An overlay has to be small or it covers the clip it is laid on, so Clear
    // drops the date and the muscle line and keeps three rows at most.
    ...
    // Over a photo or over nothing the panel goes translucent, so what is behind
    // the card still reads through the one solid shape on it.
  ...
  // The palette, the face and the marks: everything a draw needs that costs a
  // promise. The recorder resolves them once, then draws a hundred and fifty
  // times with nothing left to wait for.
  ...
  // this card already knows how to paint any moment of itself. So it records itself
  // while the poster is still reading the summary — never in the tap, where five
  // seconds of wall clock is five the sheet's activation window has not got.
  // MP4 or nothing: iOS Photos will not take a WebM and the extensions ignore it.
  var SC_MS = 5000;
    ...
    return scAssets(card).then(function (a) {
      return new Promise(function (ok, no) {
        ...
        function shut() {
          ...
          // A stream torn down under a recorder still flushing loses the last chunk.
        ...
        // A backgrounded tab stops rAF dead, so the rest of the clip would be one
        // frozen frame. No video beats a still one pretending to be a video.
  ...
  function scForget() {
    ...
    // Bumping the sequence tells a recorder still running for the summary that has
    // just closed to throw its clip away rather than arm a button nobody can see.
    sc.vseq++;
    sc.vst = 2;
    sc.vno = false;
    // The picture belonged to that session. Holding a decoded 1080-wide bitmap
    // after the summary has gone is megabytes a phone has better uses for, and a
    // new session must not quietly inherit someone's last gym selfie.
  ...
  // The clip belongs to the card on screen, so a chip tap that changes the look
  // records again; the sequence keeps a slow recorder off a card nobody can see.
  // A recorder that fails takes its own button away: nobody asked for a video, so
  // a toast about one would be noise.
  function scMakeVideo() {
    ...
    // A clip has no alpha either, so the transparent card has nothing to record;
    // its button leaves and comes back with the chip that asked for it, which is
    // a shift the poster's own thumb caused.
    if (sc.vno) return;
  ...
  // from the start and simply cannot be pressed yet: one that appeared late would
  // land under a thumb already moving towards Done.
  function scArm() {
  ...
  // The transparent card is not a post on its own — it is the thing you drop on
  // top of your own clip — and nobody knows that unless the row says so.
  function scSay() {
    if (!sc.hint) return;
  ...
  // A 48MP phone photo decoded whole is about 190MB of canvas and an older iPhone
  // kills the tab for it, so the decoder is asked for 1080 wide where it can do
  // that. A blob URL from our own origin does not taint the canvas, so toBlob
  // still comes back with a card.
  function scPhoto(f) {
  ...
  // traffic here — it is credit and discovery. A best beats volume in the first
  // line, the creator is named the way the card names them, and five hashtags is
  // Instagram's own guidance. It goes on the clipboard and never into
  // share({text}): on iOS some targets take the text and drop the file.
  function scCaption(d) {
    ...
    // A middot, not a dash: half the titles people save already carry a dash.
  ...
  function scCopyCaption() {
    ...
    // execCommand is deprecated everywhere and is still the only clipboard a
    // WebView without the async one has.
    function old() {
      t = el("textarea");
      t.value = s;
  ...
  function scButtons(btns) {
    var can = scCan();
    sc.btns = btns;
    // Above Share on purpose: by the time Instagram's caption field opens the
    // clipboard is already loaded, and the poster has one paste left to do.
  ...
  function shareRow(payload, logged) {
    ...
    // Two chips to a row and never four across: at 375px four 44px targets beside
    // a 140px preview clip. The camera roll is a hidden file input with no capture
    // attribute — capture would open the camera and skip the roll entirely.
    ...
    [["dark", "Dark"], ["light", "Light"], ["clear", "Clear"], ["photo", "Photo"]]
      .forEach(function (t) {
        ...
        b.onclick = function () {
          haptic("tap");
          // Photo always opens the roll: once a picture is chosen, changing it is
          // the only thing the chip has left to do.
