...
(function () {
  ...
  function startWorkout(w, resume) {
    ...
    wo = {
      ...
      entries: (resume && resume.entries) || screens.map(function (s) {
        return { name: s.ex.name, block: s.bi, exercise: s.ei, sets: [] };
      }),
      startedAt: (resume && resume.startedAt) || new Date().toISOString(),
      wake: null
    };
    if (!wo.entries.length) wo.entries = [{ name: "Freestyle", block: 0, exercise: 0, sets: [] }];
      ...
      .then(function (r) {
        ...
        r.data.forEach(function (log) {
          (log.entries || []).forEach(function (e) {
            if (!e.name || lastWeight[e.name] !== undefined) return;
            var last = (e.sets || []).filter(function (s) { return s.weight; }).pop();
            if (last) lastWeight[e.name] = last.weight;
          });
        });
      });
  }
  ...
  function openSetSheet(idx) {
    ...
    setCtx.weight = existing ? existing.weight : (lastWeight[entry.name] || 0);
  ...
  function renderProgress() {
    ...
    // personal records, by estimated 1RM (Epley)
    var prs = {};
    logs.forEach(function (l) {
      (l.entries || []).forEach(function (e) {
        (e.sets || []).forEach(function (s) {
          ...
          if (!prs[e.name] || est > prs[e.name].est) {
            prs[e.name] = { est: est, weight: s.weight, reps: s.reps, date: l.started_at };
          }
        });
      });
    });
    var prNames = Object.keys(prs).sort(function (a, b) { return prs[b].est - prs[a].est; }).slice(0, 12);
    if (prNames.length) {
      ...
      prNames.forEach(function (n) {
        ...
        var r = el("div", "prrow");
        var nm = el("div", "n");
        nm.appendChild(document.createTextNode(n));
