...
// ANTHROPIC_API_KEY, when set, takes precedence upstream in extractCard.
...
function haveAI(): boolean {
  return !!(ANTHROPIC_API_KEY || GEMINI_API_KEY || GROQ_API_KEY);
}
...
async function extractCard(meta: Meta, platform: string): Promise<Card> {
  ...
  const text = (await parseWithClaude(system, user)) ?? (await textGenerate(system, user, true));
  if (!text) return base;
  try {
...
async function aiText(system: string, user: string, cors: Cors): Promise<Response> {
  ...
  const out = (await parseWithClaude(system, user)) ?? (await textGenerate(system, user, false));
