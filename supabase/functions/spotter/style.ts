...
export const STYLE = String.raw`<style>
  ...
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  /* A day: its number and its dot. Today is told by colour and the selected day
     ...
     Calendar tells the two apart. */
  .cday { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 6px;
    padding: 0; border: 1px solid transparent; border-radius: 12px; background: none; --dbg: var(--paper);
    transition: transform var(--t-1) var(--e-out), background-color var(--t-2) var(--e-soft); }
  .cday:active { transform: scale(.93); }
  .cdn { font: 700 15px/1 var(--display); color: var(--ink); font-variant-numeric: tabular-nums; }
  ...
  @media (prefers-reduced-motion: reduce) {
    ...
    .planbtn:active, .segbtn, .cday, .copyweek, .shelfitem { transition: none; }
