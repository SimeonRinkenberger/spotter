...
| Layout | SpotterViewController hosts the original web interface at full height; the keyboard slides over a still frame and the page lifts the sheet or composer that owns the field on the keys' own timeline (IOS-REDESIGN.md). Capacitor Keyboard resize is none: do not re-enable its frame writes, and do not pin the web view to keyboardLayoutGuide again (the guide is only sampled, to follow interactive dismissal). Floating and split iPad keyboards cover nothing and lift nothing. Pumpy hides form-navigation accessories; other forms retain Done/Next. The web app's Inter/Cabinet typography, Pumpy layout, navigation and set sheet are retained. Status text follows color scheme. Home Screen install prompt is suppressed. |
