// Offline browser checks using the actual progress, recap and canvas share code.
...
await p.setContent('<meta name="viewport" content="width=device-width, initial-scale=1">'+built.match(/<style>[\s\S]*?<\/style>/)[0]+'<main style="padding:16px;max-width:600px;margin:auto"><div id="progressview"></div></main><div id="workout"></div>'+markup.slice(markup.indexOf('<div class="sheet" id="sessionsheet"'),markup.indexOf('<div class="sheet" id="watchsheet"')));
...
function weekStats(){return {}} function goalSetting(){return 3} function weekHero(){var n=el('div','chartcard');n.appendChild(el('h2',null,'Your week'));return n;}
function disclosure(t){var n=el('details','disclosure');n.appendChild(el('summary',null,t));n.appendChild(el('div','disclosure-body'));return n;}
...
try{
 ...
 await p.getByRole('button',{name:'Back to Progress'}).click();assert.equal(await p.evaluate(()=>sc.card),null);
 ...
 await p.evaluate(()=>{wo=null;state.logs=[];renderProgress()});assert.match(await p.locator('#progressview').innerText(),/No sessions yet/);
