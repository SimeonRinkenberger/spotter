...
//
// The web app is retired and docs/sw.js is now a kill switch: it deletes Spotter's
// caches and unregisters itself, and unregistering ends the browser's push
// subscription with it. So there is deliberately no push handler left to drive. What
// is worth holding is exactly that: no push or notificationclick handler, so a web
// push that still arrives before the unregister lands is not dressed up as a working
// reminder, and the push service answers the next send with 404/410, which the
// sender below already treats as `gone` and deletes the subscription row.
// tools/sw-harness.mjs drives the kill switch itself.

const swSrc = await Deno.readTextFile("docs/sw.js");
const listeners: Record<string, (e: unknown) => void> = {};
new Function("self", "caches", swSrc)(
  { addEventListener: (k: string, fn: (e: unknown) => void) => { listeners[k] = fn; } },
  { keys: () => Promise.resolve([]) },
);
check("the retired web worker has no push handler", !("push" in listeners));
check("and no notificationclick handler", !("notificationclick" in listeners));
check("it is the kill switch (install and activate only)",
  Object.keys(listeners).sort().join(",") === "activate,install");
